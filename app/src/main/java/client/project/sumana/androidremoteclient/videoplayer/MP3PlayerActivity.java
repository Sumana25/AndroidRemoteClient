...
public class MP3PlayerActivity extends AppCompatActivity {
    ...
    public void loadVideos() {
        HTTPRequestClient.get(Constants.getAbsoluteUrl("/do_action?action="+"music", this),
                null,
...
//                                Toast.makeText(VideoPlayerActivity.this, videos.get(i).toString(), Toast.LENGTH_SHORT).show();
                            ...
                            ArrayAdapter adapter = new ArrayAdapter<String>(MP3PlayerActivity.this, R.layout.list_item, videoList.toArray(new String[videoList.size()]));
                    ...
                    public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable error) {
                        //Failure code
                        Toast.makeText(MP3PlayerActivity.this, error.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    public void runCommand(String command, String arg) {
        HTTPRequestClient.get(Constants.getAbsoluteUrl("/do_action?action="+command+"&arg="+arg, this),
                ...
                new AsyncHttpResponseHandler() {
                    ...
                    public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                        try {
                            Toast.makeText(MP3PlayerActivity.this, new String(responseBody, "UTF-8"), Toast.LENGTH_SHORT).show();
